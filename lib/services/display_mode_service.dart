...
class DisplayModeService {
  ...
  static const String _compactLayoutKey = 'display_compact_layout';
  ...
  /// Get whether compact layout is enabled
  /// Defaults to false when not set
  Future<bool> isCompactLayout() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(_compactLayoutKey) ?? false;
  }

  /// Persist compact layout preference
  Future<void> setCompactLayout(bool isCompact) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_compactLayoutKey, isCompact);
  }

  /// Toggle compact layout and return new value
  Future<bool> toggleCompactLayout() async {
    final current = await isCompactLayout();
    final updated = !current;
    await setCompactLayout(updated);
    return updated;
  }
