...
- **Implementation**: `((playerCount + 3) ~/ 4)` for integer division with rounding up
- **Constraints**: Clamped between `Constants.minCourts` (1) and `Constants.maxCourts` (8)
...
1. **`_calculateSuggestedCourtCount(int playerCount)`**
   ...
   - Rounds up to ensure all players have a court
...
- ✅ Calculation for 1-4 players → 1 court
- ✅ Calculation for 5-8 players → 2 courts
- ✅ Calculation for 9-12 players → 3 courts
...
- ✅ Court count auto-adjusts when adding players (4→5 players triggers 1→2 courts)
- ✅ Court count auto-adjusts when removing players
- ✅ Manual adjustment still works and coexists with auto-adjustment
- ✅ Full user flow: add 9 players, verify 3 courts, remove players, verify adjustment
...
4 players → 1 court
5 players → 2 courts (auto-adjustment triggered, animation plays)
8 players → 2 courts
9 players → 3 courts (auto-adjustment triggered, animation plays)
24 players → 6 courts
```

### Automatic Adjustment on Remove
```
9 players (3 courts) → Remove 1 player → 8 players (2 courts, animation plays)
8 players (2 courts) → Remove 4 players → 4 players (1 court, animation plays)
```

### Manual Override
```
4 players (auto: 1 court) → User manually sets 3 courts
Add 5th player → Auto-adjusts to 2 courts (overrides manual setting)
